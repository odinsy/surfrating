...
rating:
	python3 ./scripts/surfrating/main.py --config	conf/rfs/config.yaml $(conf_scoring_systems) $(conf_decay_system) $(conf_years_system) conf/rfs/events.yaml conf/rfs/surfing/rus/$(discipline)_$(category).yaml | column -t -s ','
...
tvoisurf39:
	python3 ./scripts/surfrating/main.py --config	conf/tvoisurf39/config.yaml conf/tvoisurf39/events.yaml $(conf_scoring_systems) $(conf_decay_system) conf/tvoisurf39/$(discipline)_$(category).yaml | column -t -s ','
